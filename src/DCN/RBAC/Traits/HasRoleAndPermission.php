...
trait HasRoleAndPermission
{
     ...
     * @return bool
     */
    public function allowed($providedPermission, $table, $owner = false, $ownerColumn = 'user_id')
    {
        if ($this->isPretendEnabled()) {
            return $this->pretend('allowed');
        }

        if ($owner === true && DB::table($table)->where($ownerColumn, $this->id)->count() > 0) {
            return true;
        }

        return $this->isAllowed($providedPermission, $table);
    }
     ...
     * @return bool
     */
    protected function isAllowed($providedPermission, $table)
    {
        foreach ($this->getPermissions() as $permission) {
            if ($permission->model != '' && $table == $permission->model
    ...
    public function __call($method, $parameters)
    {
        if (starts_with($method, 'is')) {
            ...
            return $this->allowed(snake_case(substr($method, 7), config('roles.separator')), $parameters[0], (isset($parameters[1])) ? $parameters[1] : false, (isset($parameters[2])) ? $parameters[2] : 'user_id');
        }
